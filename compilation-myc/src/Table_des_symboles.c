...
void print_table(FILE *f){
	...
	tracker = storage;
	while(tracker){
		if (tracker->symbol_value->is_fun || tracker->symbol_value->is_param){
			tracker = tracker->next;
			continue; // ne pas reecire les fonctions
		}
		fprintf(f, "%s", tracker->symbol_value->type_val == INT ? "int" : "float");
		for(int i = 0; i < tracker->symbol_value->pointer_lvl; i++){
			fprintf(f, "*");
		}
		fprintf(f, " %s;\n", tracker->symbol_name);
		tracker = tracker -> next;
...
int valid(char * name){
  attribute att = get_symbol_value(name);
  if (att == NULL){
	  //printf("/*non*/\n");
	  return 0;
  }
  for(int i = get_hauteur(); i >= 0; i--){
	  //printf("/*bloc %d*/\n", get_pile_block(i));
    if (att->block == get_pile_block(i)){
		return 1;
	}
  }
  return 0;
